'''
Same as CNN coode but changed for RNN-LSTM implementation
'''
import json
import numpy as np 
...
def prepare_datasets(test_size, validation_size):
	...
	#For CNN - 3d array input --> (130 time bins, 13 mfcc values, 1] format
	# For RNN - dont need this additional dim
...
def build_model(input_shape):
	...
	#create RNN - LSTM model;  drop all CNN layers
	model = keras.Sequential()

	# 2 LSTM layers
	model.add(keras.layers.LSTM(64,input_shape=input_shape,return_sequences=True)) #enabling sequence to sequence
	model.add(keras.layers.LSTM(64)) #sequence to vector

	# Dense Layer + dropout for overfitting
...
if __name__ == "__main__":
	...
	# build the CNN net
	# For RNN - update this and drop the extra dim 
	#input_shape = (X_train.shape[1],X_train.shape[2],X_train.shape[3]) #130 - time steps, 13 - MFCC coefficients
	input_shape = (X_train.shape[1],X_train.shape[2])
	
	#update this to be RNN - LSTM
